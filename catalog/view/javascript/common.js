...
function spawnAlert(message = "",color="green") {
	if(color !== "green" && color !=="red" && color!=="yellow"){
		throw "Color value must be 'green' || 'red' || 'yellow'";
	}
	var alertMsg = $('<div class="alert add-to-cart-alert alert-dismissible white-background cl-black brdr-20 cl-brdr-'+color+' brdr-w-2" ><i class="fa fa-check-circle"></i> ' + message + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
	$('body').append(alertMsg);
	alertMsg.addClass('cart-alert-slide__down');
	setTimeout(function () {
		alertMsg.toggleClass('cart-alert-slide__down');
	}, 3000)
}
		...
		$.ajax({
			...
			success: function (json) {
				...
				if (json['success']) {
					...
					spawnAlert(json['success'],"yellow");
				}
